...
namespace MiniShop.Business.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public ResponseDto<List<CategoryDto>> GetAll(bool? isActive, bool? isDeleted)
        {
            var response = new ResponseDto<List<CategoryDto>>();
            List<Category> categories = _categoryRepository.GetActiveAndUndeletedCategories(isActive, isDeleted);
            if (categories.Count == 0)
            {
                response.Data = null;
                response.Error = "Hiç kategori bulunamadı.";
        ...
        public ResponseDto<CategoryDto> Update(UpdateCategoryDto updateCategoryDto)
        {
            var updatedCategory = _mapper.Map<Category>(updateCategoryDto);
            updatedCategory.ModifiedDate = DateTime.Now;
            var resultCategory = _categoryRepository.Update(updatedCategory);
            return new ResponseDto<CategoryDto>
            {
                Data = _mapper.Map<CategoryDto>(resultCategory),
                Error = null
